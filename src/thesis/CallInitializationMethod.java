...
/************************************************************************************************/
public class CallInitializationMethod
/************************************************************************************************/
...
/************************************************************************************************/
	    ...
	    public CallInitializationMethod(MemHelper helper)    
	    {
	        this.helper   = helper;
...
//	    	MAX_ITERATIONS  = iterations;
	    	...
	    	finalClusters 	= constructRecTreeM	(methodVariant, helper.getListOfUsers(), 
	    										howManyClusters, 
	...
	/************************************************************************************************/
	    ...
	    public ArrayList<IntArrayList> constructRecTreeM(int variant , IntArrayList dataset,    // helper.getListOfUsers(),
	                                                     int currDepth,           // k,  
	                                                     double cliqueAverage)    // helper.getGlobalAverage());                                              
	    {
	       ArrayList<IntArrayList> clusters = new ArrayList<IntArrayList>(currDepth);
	               
	        ClusterCollection subClusters = kMeans ( variant , dataset, 	
	        										howManyClusters, 
	        									    cliqueAverage);
	...
	/************************************************************************************************/
		...
		public ClusterCollection kMeans(int variant, IntArrayList dataset, 		//all users in the database
	    								int k, 					
	                                    double cliqueAverage) 		//golbal average in the database    
	    {
	        ...
	        methodVariant = variant;
//	        System.out.println("dataset is :::" +dataset + "cluster" +howManyClusters);
	      ...
	      //Initialise the centroids as k random points in the dataset.
	        ...
	        {     
	        	timer.start();	 
	        	if (methodVariant==1)
	        		returnedCentroids = simpleKTree.chooseRandomCentroids(dataset, howManyClusters);      
	        	if (methodVariant==2)
		        	this.centroids = simpleKPlusTree.choosePlusCentroids(dataset, howManyClusters);
	        	if (methodVariant==3)
		        	this.centroids = simpleKModifiedPlusTree.chooseModifiedPlusCentroids(dataset, howManyClusters);
	        	if (methodVariant==4)
		        	this.centroids = simpleKPlusAndPowerTree.choosePlusPowerCentroids(dataset, howManyClusters);
//	        	if (methodVariant==5)
//		        	this.centroids = simpleKTree.chooseRandomCentroids(dataset, howManyClusters);
	        	if (methodVariant==9)
		        	this.centroids = simpleKDistance.chooseQuantileCentroids(dataset, howManyClusters);
	        	if (methodVariant==10)
		        	this.centroids = simpleKNormal.chooseNormalCentroids(dataset, howManyClusters);
	        	if (methodVariant==11)
		        	this.centroids = simpleVariance.chooseVarianceCentroids(dataset, howManyClusters);
	        	if (methodVariant==12)
		        	this.centroids = simpleUniform.chooseUniformCentroids(dataset, howManyClusters);
	        	if (methodVariant==13)
		        	this.centroids = simpleDensity.chooseDensityCentroids(dataset, howManyClusters);
	        	if (methodVariant==14)
		        	this.centroids = simpleSample.chooseSampleCentroids(dataset, howManyClusters);
	        	if (methodVariant==15)
		        	this.centroids = simpleLog.chooseLogCentroids(dataset, howManyClusters);
	        	if (methodVariant==16)
		        	this.centroids = simpleHGeo.chooseHyperGeoCentroids(dataset, howManyClusters);
	        	if (methodVariant==17)
		        	this.centroids = simplePoisson.choosePoissonCentroids(dataset, howManyClusters);
	        	if (methodVariant==18)
		        	this.centroids = simplePlusPlus.chooseKPlusPlusCentroids(dataset, howManyClusters);
		        timer.stop();   
		        
		        this.centroids =returnedCentroids;
